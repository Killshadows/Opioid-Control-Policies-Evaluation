...
+ geom_line()
+ labs(x = "Year", y = "Drug Deaths Per Cap", title = "States Deaths Trend"))
...
## Heat map
# Get Deaths per ten thousand people on state level
mortality_pop_norm.head()
mortality_state = mortality_pop_norm.groupby(["State"]).mean().reset_index()
mortality_state['Deaths_PerTenThousand_State'] = mortality_state['Deaths_PerCap_County']*10000
mortality_state

import plotly.graph_objects as go

fig = go.Figure(data=go.Choropleth(
    locations=mortality_state['State'], # Spatial coordinates
